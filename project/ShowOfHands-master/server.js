...
app.get('/getPage', function(req, res) {
	...
	var bill_args = req.query.id.split("-"); //splitt the bill id into the bill number and congress number
        //console.log(docs[j]);
        congressClient.billDetails({
          congressNumber: bill_args[1],
          billId: bill_args[0]
        }).then(function(data) { //grab details about the bill
          console.log(data.results[0].congressdotgov_url);
          var site = data.results[0].congressdotgov_url; //get the congressdotgov url to send back to angular

          res.send(site); //send site
...
app.get('/bills', function(req, res){
	...
	//find the correct ammount of bills and skip a few every time so it doesnt show all from the house or from the senate
  var query = Bill.find({});
